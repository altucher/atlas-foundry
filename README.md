...
The home-page header includes a 27-second narrated product trailer demonstrating the explosion slider, component records, data-center research layers, supplier evidence, public-company tickers, and finance links.
...
2. The browser receives a fast first draft as soon as the component architecture and assembled/exploded image pair are ready. Every component and the explosion slider are usable immediately. A second request then divides the full parts list into small parallel supplier-research batches and updates the open atlas automatically when the audit finishes. It searches for current, former, alternative, generation-specific, factory-specific, regional, and credibly rumored relationships instead of spending the whole search budget on headline chips. It distinguishes manufacturer, assembler, designer, IP licensor, software provider, material supplier, and integrator roles—for example, display panel/module/backlight/driver roles and battery cell/pack/BMS roles remain separate, and a chip designer is not treated as its foundry or packaging/test provider.
...
4. The Images API renders matched high-quality assembled and exhaustive exploded studio views in parallel during the first stage, using portrait plates for strongly vertical subjects such as launch vehicles. Supplier research never blocks those images from appearing.
...
### Multilevel Falcon 9 archive

Falcon 9 can be deepened into six evidence-bounded research layers after its overview exists: Airframe & tanks, Merlin & propulsion, Staging & payload, Avionics & flight, Recovery & reuse, and Ground & mission interfaces. The research distinguishes the current Block 5 family from earlier generations and mission-specific configurations, decomposes documented subassemblies, and runs a component-level supplier/IP audit on every layer. The prompts `Falcon 9`, `Falcon Nine`, `SpaceX Falcon 9`, and `SpaceX Falcon Nine` share the canonical `falcon-9` gallery record. Maintainers use the same `X-Atlas-Deep-Build: 1` request header to extend it.
...
The generation route requests an extended Fluid-compute window. GPT-5.4 mini performs the first component pass with low reasoning and a bounded search context; GPT Image 2.5 Flare renders the two high-quality views in parallel. That draft is saved separately and returned before the forensic work begins. The browser immediately starts a background enrichment request: six-component supplier batches use GPT-6 Astra for evidence depth while precise visual localization runs alongside them. Optional deep archive layers keep GPT-5.6 Terra for source-backed decomposition. Responses are submitted in background mode and polled when the provider supports that lifecycle; if a transient upstream disconnect occurs, the route automatically retries with a bounded source window. The deployment plan must support the configured duration; completed subjects subsequently load from the shared gallery.
