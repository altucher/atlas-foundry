...
export default function AnatomyScene({atlas,state,onSelect,onProgress,onError}:Props){
 ...
 useEffect(()=>{
  ...
  renderer.setPixelRatio(Math.min(devicePixelRatio,innerWidth<768?1.5:2));renderer.setClearColor('#090a09');renderer.outputColorSpace=T.SRGBColorSpace;renderer.toneMapping=T.ACESFilmicToneMapping;renderer.toneMappingExposure=1.18;el.appendChild(renderer.domElement);
  ...
  scene.add(new T.HemisphereLight(0xcfc8b8,0x111713,.72));
  const key=new T.DirectionalLight(0xfff1d6,3.4);key.position.set(-2.8,4.5,3.2);scene.add(key);
  const fill=new T.DirectionalLight(0x9aaea6,1.25);fill.position.set(3,1.2,2);scene.add(fill);
  const rim=new T.DirectionalLight(0xd9dfd5,2.6);rim.position.set(2.4,2.8,-3.8);scene.add(rim);
  const ground=new T.Mesh(new T.CircleGeometry(30,96),new T.MeshStandardMaterial({color:0x0b0c0b,roughness:1,metalness:.05}));ground.rotation.x=-Math.PI/2;ground.position.y=-.019;scene.add(ground);
  const platform=new T.Mesh(new T.CylinderGeometry(.68,.7,.028,100),new T.MeshStandardMaterial({color:0x171815,metalness:.34,roughness:.56}));platform.position.y=-.016;scene.add(platform);
  const ring=new T.Mesh(new T.RingGeometry(.63,.632,128),new T.MeshBasicMaterial({color:0xc9b993,transparent:true,opacity:.42,side:T.DoubleSide}));ring.rotation.x=-Math.PI/2;ring.position.y=.001;scene.add(ring);
  const innerRing=new T.Mesh(new T.RingGeometry(.55,.551,128),new T.MeshBasicMaterial({color:0xc9b993,transparent:true,opacity:.12,side:T.DoubleSide}));innerRing.rotation.x=-Math.PI/2;innerRing.position.y=.001;scene.add(innerRing);
  ...
  const markerMaterial=new T.PointsMaterial({color:0xc8bc9f,size:4,sizeAttenuation:false,transparent:true,opacity:.58,depthTest:false});
  ...
  const materialFor=(system:string)=>{
   const m=new T.MeshStandardMaterial({color:SYSTEMS.find(s=>s.id===system)?.color??'#b9ad95',metalness:.11,roughness:.62,side:T.DoubleSide,transparent:system==='integumentary',opacity:system==='integumentary'?.1:1,depthWrite:system!=='integumentary'});
   m.onBeforeCompile=shader=>{
    ...
    shader.fragmentShader=shader.fragmentShader.replace('#include <color_fragment>','#include <color_fragment>\ndiffuseColor.rgb = mix(diffuseColor.rgb, vec3(0.96, 0.77, 0.34), partSelected * 0.78);');
